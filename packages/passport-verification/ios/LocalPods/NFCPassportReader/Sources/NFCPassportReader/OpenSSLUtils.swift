...
public class OpenSSLUtils {
    ...
    static func getX509CertificatesFromPKCS7( pkcs7Der : Data ) throws -> [X509Wrapper] {
        ...
        var ret = [X509Wrapper]()
        if let certs = certs  {
            let certCount = OpenSSLLocal_sk_X509_num(certs)
            for i in 0 ..< certCount {
                let x = OpenSSLLocal_sk_X509_value(certs, i);
    ...
    static func verifyTrustAndGetIssuerCertificate( x509 : X509Wrapper, CAFile : URL ) -> Result<X509Wrapper, OpenSSLError> {
        ...
        let nrCertsInChain = OpenSSLLocal_sk_X509_num(chain)
        if nrCertsInChain > 1 {
            let cert = OpenSSLLocal_sk_X509_value(chain, nrCertsInChain-1)
